...
def start_http_server():
    ...
    print(f"HTTP server pe portul {port}")
    httpd.serve_forever()
...
def make_survivor(pid, name):
    ...
    return {
        ...
        "coins":0,
        "colaBuff":0.0,"colaCooldown":0.0,
        "stamina":100,"maxStamina":100,"staminaRegen":1,
        "bleeding":0.0,"bleedTimer":0.0,
    }
        ...
        if game_phase == "PLAYING":
            ...
            for c in clients.values():
                ...
                if not p.get("isSprinting", False):
                    p["stamina"] = min(p.get("maxStamina",100),
            ...
            if killer_rec:
                ...
                if axe["flying"]:
                    ...
                    if hit_wall and not hit_pid:
                        ...
                        s["hp"] -= 25; s["invul"] = 60
                        s["bleeding"] = 10.0; s["bleedTimer"] = 0.0
            ...
            await broadcast({
                "type":"state","timer":round_timer,"walls":walls,
                "players":{pid:serialize(c["data"]) for pid,c in clients.items()},
            })
...
async def handle_input(pid: str, msg: dict):
    ...
    if mtype == "move":
        ...
        d["stamina"] = max(0, d.get("stamina",100)-float(msg.get("amount",0)))
        d["isSprinting"] = True
        ...
        if d.get("colaCooldown",0) <= 0:
            d["speed"] = 9; d["staminaRegen"] = 2
            d["colaBuff"] = 5.0; d["colaCooldown"] = COLA_COOLDOWN
...
async def handler(ws):
    ...
    try:
        ...
        await send_to(pid, {
            "type":"welcome","pid":pid,"walls":walls,"phase":game_phase,
